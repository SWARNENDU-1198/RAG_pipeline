"""
build_vast_msmarco_xi.py

Connects to Hugging Face dataset 'ai4bharat/MSMARCO-XI' and extracts a vast,
diverse corpus of questions, answers, and passages across all 12+ Indic languages + English.
Ensures accurate parsing of 'Translated_passages', 'English_passages', 'query', 'Eng_Query', 'Answer', 'Eng_Answer'.
"""
...
# Parquet file mapping for all 12 Indic languages in MSMARCO-XI
LANGUAGE_PARQUETS = {
...
def build_vast_corpus(queries_per_indic_lang: int = 180, max_passages: int = 8000) -> Dict[str, Any]:
    # Import fallback domain samples
    ...
    logger.info(f"Extracting vast balanced MSMARCO-XI corpus ({queries_per_indic_lang} queries per language, max passages: {max_passages})...")
        ...
        try:
            ...
            for idx, item in enumerate(ds_stream):
                ...
                for p_idx, p_text in enumerate(translated_passages):
                    ...
                    if p_clean not in seen_passages and len(all_passages) < max_passages:
    ...
    dataset_dict = {
        ...
        "metadata": {
            ...
            "total_passages": len(all_passages[:max_passages]),
    ...
    logger.info(f"Successfully wrote vast dataset to {OUTPUT_FILE}: {len(all_queries)} queries, {len(all_passages[:max_passages])} passages.")
    logger.info(f"Final language distribution: {lang_query_counts}")
    return dataset_dict


if __name__ == "__main__":
    build_vast_corpus(queries_per_indic_lang=200, max_passages=8000)
